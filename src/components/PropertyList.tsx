...
const PropertyCard: React.FC<Property> = ({
  ...
  <div className="border rounded-xl bg-white p-6 shadow hover:shadow-lg transition flex flex-col min-h-[340px] relative">
    <span
      className={`absolute top-6 right-6 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${typeColors[property] || typeColors.OTHERS}`}
    >
      {property}
    </span>
    <div>
      <div className="mb-2"><b>Bank:</b> {bank_name}</div>
      <div className="mb-2"><b>Reserve Price:</b> {reserve_price}</div>
      <div className="mb-2"><b>EMD:</b> {emd}</div>
      <div className="mb-2"><b>EMD Last Date:</b> {emd_last_date || 'TBA'}</div>
      <div className="mb-2"><b>Auction Start:</b> {auction_start_date || 'TBA'}</div>
      <div className="mb-2"><b>Auction End:</b> {auction_end_date || 'TBA'}</div>
    ...
    query.then(({ data, error }) => {
      ...
      if (selectedBudget) {
        if (selectedBudget.endsWith('+')) {
          ...
          filtered = filtered.filter(p => parseRupee(p.reserve_price) >= min);
        } else if (selectedBudget.includes('-')) {
          const [min, max] = selectedBudget.split('-').map(x => parseInt(x, 10));
          filtered = filtered.filter(p => {
            const price = parseRupee(p.reserve_price);
